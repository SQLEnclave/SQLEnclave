...
public struct DatabaseMigrator {
    ...
    public func appliedIdentifiers(_ db: Database) throws -> Set<String> {
        do {
            return try Set(String.fetchCursor(db, sql: "SELECT identifier FROM sqldb_migrations"))
        } catch {
            // Rethrow if we can't prove sqldb_migrations does not exist yet
            if (try? !db.tableExists("sqldb_migrations")) ?? false {
                return []
            }
            throw error
        }
    ...
    private func runMigrations(_ db: Database, upTo targetIdentifier: String) throws {
        ...
        try db.execute(sql: "CREATE TABLE IF NOT EXISTS sqldb_migrations (identifier TEXT NOT NULL PRIMARY KEY)")
            ...
            try db.inTransaction(.deferred) {
                    ...
                    .last(where: { appliedIdentifiers.contains($0) })
                {
                    ...
                    let tmpSchema: SchemaInfo = try {
                        ...
                        tmpConfig.label = "SQLEnclave.DatabaseMigrator.temporary"
