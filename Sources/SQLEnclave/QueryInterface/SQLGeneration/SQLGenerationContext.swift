...
public class TableAlias: Hashable {
    private enum Impl {
        /// A TableAlias is undefined when it is created by the SQLEnclave user:
        ///
...
extension Array where Element == TableAlias {
    ...
    fileprivate var resolvedNames: [TableAlias: String] {
        ...
        //          .including(required: Book.author.aliased(alias)...)
        SQLEnclavePrecondition(count == Set(self).count, "A TableAlias most not be used to refer to multiple tables")
