...
extension QueryInterfaceRequest {
    ...
    private var defaultConflictResolutionForUpdate: Database.ConflictResolution {
        ...
        // user-defined default. Such dynamic dispatch is unusual in SQLEnclave, but
    ...
    into originRows: [Row]) throws
{
    ...
    for association in associations {
        ...
        case .expression:
            // Likely a SQLEnclave bug: such condition only exist for CTEs, which
        ...
        case let .foreignKey(pivotForeignKey):
            ...
            //
            //      WITH sqldb_base AS (SELECT a, b FROM parent)
            //      SELECT * FROM child
            //      WHERE (a, b) IN sqldb_base
            //
            ...
            if usesCommonTableExpression {
                ...
                //
                //      WITH sqldb_base AS (SELECT a, b FROM author)
                //      SELECT book.*, book.authorId AS sqldb_authorId
                //      FROM book
                //      WHERE (book.a, book.b) IN sqldb_base
                //
                // HasManyThrough: Citizen.including(all: Citizen.countries)
                //
                //      WITH sqldb_base AS (SELECT a, b FROM citizen)
                //      SELECT country.*, passport.citizenId AS sqldb_citizenId
                //      FROM country
                //      JOIN passport ON passport.countryCode = country.code
                //                    AND (passport.a, passport.b) IN sqldb_base
                //
                ...
                // HasMany: Author.including(all: Author.books)
                //
                //      SELECT *, authorId AS sqldb_authorId
                //      FROM book
            ...
            let prefetchedGroups = prefetchedRows.grouped(byDatabaseValuesOnColumns: pivotColumns.map { "sqldb_\($0)" })
...
-> QueryInterfaceRequest<Row>
{
    ...
    let prefetchRelation = association
        ...
        .destinationRelation()
        .annotated(with: pivotColumns.map { pivotAlias[$0].forKey("sqldb_\($0)") })
