...
public struct Configuration {
    ...
    /// created by SQLEnclave, visible in debugging sessions and crash logs.
    ///
    /// Those connection names and dispatch queue labels are intended for
    /// debugging only. Their format may change between SQLEnclave releases.
    /// Applications should not depend on connection names and dispatch
    /// queue labels.
    ///
    /// If the configuration label is nil, the current SQLEnclave implementation uses
    /// the following names:
    ///
    /// - `SQLEnclave.DatabaseQueue`: the (unique) connection of a DatabaseQueue
    /// - `SQLEnclave.DatabasePool.writer`: the (unique) writer connection of
    ///   a DatabasePool
    /// - `SQLEnclave.DatabasePool.reader.N`, where N is 1, 2, ...: one of the reader
    ...
    ///   ones are opened.
    /// - `SQLEnclave.DatabasePool.snapshot.N`: the connection of a DatabaseSnapshot.
    ///   N grows with the number of snapshots.
    ///
    /// If the configuration label is not nil, for example "MyDatabase", the
    /// current SQLEnclave implementation uses the following names:
    ///
    ...
    /// If nil, SQLEnclave picks a default one.
