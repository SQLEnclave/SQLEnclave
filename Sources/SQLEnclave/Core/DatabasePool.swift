...
public final class DatabasePool: DatabaseWriter {
    ...
    public init(path: String, configuration: Configuration = Configuration()) throws {
        SQLEnclavePrecondition(configuration.maximumReaderCount > 0, "configuration.maximumReaderCount must be at least 1")
        
        // Writer
        writer = try SerializedDatabase(
            path: path,
            configuration: configuration,
            defaultLabel: "SQLEnclave.DatabasePool",
            purpose: "writer")
        ...
        // See https://github.com/sqlenclave/SQLEnclave/#memory-management
        #if os(iOS)
...
extension DatabasePool: DatabaseReader {
    ...
    public func read<T>(_ block: (Database) throws -> T) throws -> T {
        SQLEnclavePrecondition(currentReader == nil, "Database methods are not reentrant.")
    ...
    public func asyncConcurrentRead(_ block: @escaping (Result<Database, Error>) -> Void) {
        ...
        writer.execute { db in
            ...
            SQLEnclavePrecondition(!db.isInsideTransaction, """
