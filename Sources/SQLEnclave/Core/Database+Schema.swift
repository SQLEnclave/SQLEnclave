extension Database {
    ...
    /// Returns whether a table is an internal SQLEnclave table.
    ///
    /// Those are tables whose name begins with "sqldb_".
    public static func isSQLEnclaveInternalTable(_ tableName: String) -> Bool {
        tableName.starts(with: "sqldb_")
    }
    
    /// Returns whether a table is an internal SQLEnclave table.
    ///
    /// Those are tables whose name begins with "sqldb_".
    @available(*, deprecated, message: "Use Database.isSQLEnclaveInternalTable(_:) static method instead.")
    public func isSQLEnclaveInternalTable(_ tableName: String) -> Bool {
        Self.isSQLEnclaveInternalTable(tableName)
    }
            ...
            .compactMap { row -> IndexInfo? in
                ...
                for indexInfoRow in indexInfoRows {
                    guard let column = indexInfoRow[2] as String? else {
                        ...
                        // Maybe in a future SQLEnclave version?
                        return nil
                    }
