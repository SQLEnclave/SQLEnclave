...
public final class Database: CustomStringConvertible, CustomDebugStringConvertible {
    ...
    private func validateSQLCipher() throws {
        ...
        if try String.fetchOne(self, sql: "PRAGMA cipher_version") == nil {
            throw DatabaseError(resultCode: .SQLITE_MISUSE, message: """
                SQLEnclave is not linked against SQLCipher. \
    ...
    private func validateFormat() throws {
        ...
        // another passphrase.
        _ = try makeStatement(sql: "SELECT * FROM sqlite_master LIMIT 1").makeCursor().next()
    }
    ...
    private static func closeConnection(_ sqliteConnection: SQLiteConnection) {
        ...
        if code != SQLITE_OK, let log = logError {
            // A rare situation where SQLEnclave doesn't fatalError on
            // unprocessed errors.
    ...
    public func add(collation: DatabaseCollation) {
        ...
        guard code == SQLITE_OK else {
            // Assume a SQLEnclave bug: there is no point throwing any error.
    ...
    ///
    /// [**Experimental**](http://github.com/sqlenclave/SQLEnclave#what-are-experimental-features)
    public static let suspendNotification = Notification.Name("SQLEnclave.Database.Suspend")
    
    /// When this notification is posted, databases which were opened with the
    /// `Configuration.observesSuspensionNotifications` flag are resumed.
    ///
    /// [**Experimental**](http://github.com/sqlenclave/SQLEnclave#what-are-experimental-features)
    public static let resumeNotification = Notification.Name("SQLEnclave.Database.Resume")
    ...
    public func inSavepoint(_ block: () throws -> TransactionCompletion) throws {
        if !isInsideTransaction {
            ...
            //
            // But SQLEnclave database configuration mandates a default transaction
        ...
        // the user uses "sqldb" as a savepoint name.
        try execute(sql: "SAVEPOINT sqldb")
        ...
        do {
            ...
            case .commit:
                ...
                try execute(sql: "RELEASE SAVEPOINT sqldb")
        ...
        if needsRollback {
            do {
                ...
                try execute(sql: "ROLLBACK TRANSACTION TO SAVEPOINT sqldb")
                try execute(sql: "RELEASE SAVEPOINT sqldb")
            } catch {
                if firstError == nil {
                    firstError = error
...
extension Database {
    ...
    ///
    /// The BusyMode enum describes the behavior of SQLEnclave when such a situation
    /// occurs:
    ///
