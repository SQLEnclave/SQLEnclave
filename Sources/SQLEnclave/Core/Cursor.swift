...
extension Cursor {
    ...
    public func dropLast(_ n: Int) throws -> [Element] {
        SQLEnclavePrecondition(n >= 0, "Can't drop a negative number of elements from a cursor")
    ...
    public func suffix(_ maxLength: Int) throws -> [Element] {
        SQLEnclavePrecondition(maxLength >= 0, "Can't take a suffix of negative length from a cursor")
        if maxLength == 0 {
            return []
        }
...
public final class DropFirstCursor<Base: Cursor>: Cursor {
    ...
    init(_ base: Base, limit: Int) {
        SQLEnclavePrecondition(limit >= 0, "Can't drop a negative number of elements from a cursor")
        self.base = base
        self.limit = limit
    }
