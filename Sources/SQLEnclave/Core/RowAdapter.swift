...
/// working together.
///
/// SQLEnclave ships with four concrete types that adopt the RowAdapter protocol:
///
...
public struct SuffixRowAdapter: RowAdapter {
    ...
    public init(fromIndex index: Int) {
        SQLEnclavePrecondition(index >= 0, "Negative column index is out of range")
        self.index = index
    }
...
public struct RangeRowAdapter: RowAdapter {
    ...
    public init(_ range: CountableRange<Int>) {
        SQLEnclavePrecondition(range.lowerBound >= 0, "Negative column index is out of range")
        self.range = range
    }
