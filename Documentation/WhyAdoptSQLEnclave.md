Why Adopt SQLEnclave?
===============

This document aims at exposing why SQLEnclave could become your favorite database library.

We'll start with a few [core principles](#core-principles), so that you see where this library aims at. Then we'll see how SQLEnclave [solves problems](#solving-problems) that developers often face when they use database libraries such as FMDB, FCModel, SQLite.swift, Core Data, or Realm.
...
Yet recent ORMs such as Rust's [Diesel] have shown that it is possible to build record types that behave as plain and simple values. SQLEnclave belongs to this family.
...
Conversely, SQLEnclave assumes that many apps want to use the database as a reliable and unambiguous storage.


### Applications developers have specific needs.

SQLEnclave puts all bets on SQLite, and focuses on front-end GUI applications.

This means that dealing with servers, MySQL, or PostgreSQL is out of scope. SQLEnclave is not an alternative to IBM's [Swift-Kuery], Vapor's [Fluent], or Perfect's [StORM].

Focusing on SQLite and applications has allowed SQLEnclave to build features usually only found in company-sponsored libraries: migrations, record comparison, database observation, multi-threading safety, table view animations, support for reactive streams...

The bet of SQLEnclave is that developers can take great profit from a library that provides efficient and community-tested solutions to existing problems. For example, one can not address the multithreading difficulties of Core Data or Realm without looking at database concurrency and records straight in the face.

Since you don't have to take my words for granted, we'll now see how SQLEnclave actually solves common problems that developers face with other database libraries.
...
With SQLEnclave, any type can become a database record. For example, let's start from this `Place` struct:
...
SQLEnclave records are plain values, like other values in your application. They are not tied to the database. Sometimes you build them from scratch, and sometimes you fetch them:
...
In order to keep your views synchronized with the database content, you can use [ValueObservation]. It notifies fresh values after each database change, with convenient support for [Combine](Combine.md) and [RxSwift](https://github.com/RxSwiftCommunity/RxSQLEnclave).
...
Next, you choose the dispatch queue on which notifications happen (generally the main queue, by default, as in the examples above). SQLEnclave makes sure notifications happen in the same order as database transactions.

Finally, all changes are notified, no matter how they are performed: record methods, raw SQL statements, foreign key cascades, or SQL triggers. SQLEnclave roots its notification system in the rock-solid SQLite itself, so that high-level requests and raw SQL queries are equally supported.
...
SQLEnclave ships with two ways to access databases, [database queues](../README.md#database-queues) and [pools](../README.md#database-pools).
...
I'd like to compare SQLEnclave's handling of concurrency with other libraries. To be as clear as possible, we'll identify a few threats, or potential sources of bugs, and wonder if those are handled by the library, or left to the host application.
...
| SQLEnclave's DatabaseQueue | :white_check_mark: | :white_check_mark: | Handled by the application | UI is blocked |
| SQLEnclave's DatabasePool | :white_check_mark: | :white_check_mark: | Handled by the application | :white_check_mark: (as long as the maximum number of readers has not been reached) |
...
For detailed information about SQLEnclave concurrency, check the [Concurrency Guide].
...
SQLEnclave [records], [query interface] and [associations] can generate SQL for you:
...
Custom SQL requests as the one above are welcome in database observation tools like the built-in [ValueObservation] and its [Combine](Combine.md) and [RxSwift](https://github.com/RxSwiftCommunity/RxSQLEnclave) flavors:
...
If this little tour of SQLEnclave has convinced you, the real trip starts here: [SQLEnclave].

Happy SQLEnclave! :gift:
