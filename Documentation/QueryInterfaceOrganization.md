...
<img src="https://github.com/sqlenclave/SQLEnclave/raw/development/Documentation/Images/QueryInterfaceOrganization2.png" width="100%">
...
- Database observation: When a request is limited to a known list of rowids in a database table, changes applied to other rows do not trigger the observation. SQLEnclave needs column expressions in order to apply this optimization:
...
-  SQL generation: when it generates SQL queries, SQLEnclave appends `LIMIT 1` or not, depending on the primary key and unique indexes used on the queried table. SQLEnclave needs column expressions in order to improve its SQL generation:
...
`QueryInterfaceRequest` is the type of fetch requests built by the SQLEnclave query builder. It conforms to [FetchRequest] and [DerivableRequest].
...
`SQL` is the type for SQL literals that support [SQL Interpolation]. It can feed all SQLEnclave APIs that have a `literal` argument:
...
When it looks like SQLEnclave APIs are unable to build a particular expression, use [SQL]:
...
Use SQLSpecificExpressible when you want to operate on expressions, except [Int, String, Date] and other types which are not directly related to SQL. For example, the built-in `length(_:)` SQLEnclave function accepts SQLSpecificExpressible:
...
SQLSubqueryable provides the SQLEnclave support for subqueries. Its [SQLSpecificExpressible] facet lets you use any request as an expression:
