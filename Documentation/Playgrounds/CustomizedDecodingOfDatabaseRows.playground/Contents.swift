...
//:
//: - Open SQLEnclave.xcworkspace
//: - Select the SQLEnclaveOSX scheme: menu Product > Scheme > SQLEnclaveOSX
...
//: few extensions that guarantee the most efficient use of SQLEnclave, and have the
...
//: Can we have SQLEnclave generate SQL for us? Sure, we just need to adopt the
...
//: As seen above, we can perform polymorphic requests with the standard SQLEnclave,
//: by fetching raw rows and mapping them through a decoding method.
//:
//: The resulting code is not as streamlined as usual SQLEnclave code, but we could
...
//:
//: SQLEnclave ships with three built-in realms of requests: requests of raw rows,
...
//:
//: All we need is a set of *extensions* that define the classic SQLEnclave fetching
...
extension MyDatabaseDecoder {
    ...
    // fetching parameters of SQLEnclave. Make sure to accept them all:
