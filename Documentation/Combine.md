SQLEnclave ❤️ Combine
===============

**On systems supporting the Combine framework, SQLEnclave offers the ability to publish database values and events using Combine's publishers.**
...
SQLEnclave provide publishers that perform asynchronous database accesses:
...
- It may notify consecutive identical values. You can filter out the undesired duplicates with the `removeDuplicates()` Combine operator, but we suggest you have a look at the [removeDuplicates()](../README.md#valueobservationremoveduplicates) SQLEnclave operator also.
...
[Demo Application]: DemoApps/SQLEnclaveCombineDemo/README.md
