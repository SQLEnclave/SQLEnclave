Contributing to SQLEnclave
====================

**Thank you for coming!**

This guide is a set of tips and guidelines for contributing to the GitHub repository [sqlenclave/SQLEnclave](https://github.com/sqlenclave/SQLEnclave).
...
Please make sure the bug is not already reported by searching existing [issues](https://github.com/sqlenclave/SQLEnclave/issues).

If you're unable to find an existing issue addressing the problem, [open a new one](https://github.com/sqlenclave/SQLEnclave/issues/new). Be sure to include a title and clear description, as much relevant information as possible, and a code sample or an executable test case demonstrating the expected behavior that is not occurring.
...
- the [answered questions](https://github.com/sqlenclave/SQLEnclave/issues?utf8=✓&q=label%3Aquestion+)
...
If not, contact [@sqlenclave](http://twitter.com/sqlenclave) on Twitter, or [open a new issue](https://github.com/sqlenclave/SQLEnclave/issues/new).
...
SQLEnclave is free. It is openly developed by its contributors, on their free time, according to their will, needs, and availability. It is not controlled by any company.

When you have specific development or support needs, and are willing to financially contribute to SQLEnclave, please send an email to [Gwendal Roué](mailto:gr@pierlis.com) so that we can enter a regular business relationship through the [Pierlis](http://pierlis.com/) company, based in Paris, France.


## Suggested Contributions

You'll find below various ideas for enhancing and extending SQLEnclave, in various areas. Your ideas can be added to this list: [Suggest an Enhancement].

Legend:

- :baby: **Starter Task**: get familiar with SQLEnclave internals
...
- :hammer: **Tooling**: SQLEnclave and its environment
...
SQLEnclave has a strong focus on safe concurrency. Not only safe as "does not crash", but safe as "actively protects your application data". The topic is discussed in (too) many places:

- [Concurrency Guide](README.md#concurrency)
- [Why Adopt SQLEnclave?](https://github.com/sqlenclave/SQLEnclave/blob/master/Documentation/WhyAdoptSQLEnclave.md#strong-and-clear-multi-threading-guarantees)
...
- [Good Practices for Designing Record Types](https://github.com/sqlenclave/SQLEnclave/blob/master/Documentation/GoodPracticesForDesigningRecordTypes.md#fetch-in-time)
...
- Better documentation of SQLEnclave concurrency
...
Applications can define their own [custom FTS5 auxiliary functions](https://www.sqlite.org/fts5.html#custom_auxiliary_functions) with SQLite, but SQLEnclave does not yet provide any Swift API for that.
...
Functions are defined in [SQLEnclave/QueryInterface/Support/SQLFunctions.swift](https://github.com/sqlenclave/SQLEnclave/blob/master/SQLEnclave/QueryInterface/Support/SQLFunctions.swift).
...
NSNumber and NSDecimalNumber support is found in [SQLEnclave/Core/Support/Foundation/NSNumber.swift](https://github.com/sqlenclave/SQLEnclave/blob/master/SQLEnclave/Core/Support/Foundation/NSNumber.swift)
...
- English: the documentation has mostly been written by [@sqlenclave](http://github.com/sqlenclave) who is not a native English speaker.
...
There exists a SQLEnclave demo app for the FTS5 full-text engine: [WWDCCompanion](https://github.com/sqlenclave/WWDCCompanion).
...
Swift on Linux is currently focused on the server (Vapor, Perfect, Kitura). While general server support is a [non-goal](#non-goals) of SQLEnclave, there exists read-only servers, and Linux GUI applications, too. Linux is thus a desired platform.
...
There are several SQLite features that SQLEnclave could natively support:
...
It would be interesting to see what typed expressions could bring to SQLEnclave.


## Non-Goals

SQLEnclave is a "toolkit for SQLite databases, with a focus on application development".

This definition is the reason why SQLEnclave can provide key features such as sharp multi-threading, database observation, and first-class support for raw SQL.
...
[RxSQLEnclave]: http://github.com/RxSwiftCommunity/RxSQLEnclave
