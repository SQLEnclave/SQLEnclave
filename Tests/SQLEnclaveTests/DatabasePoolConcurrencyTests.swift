...
class DatabasePoolConcurrencyTests: SQLEnclaveTestCase {
    ...
    func testDefaultLabel() throws {
        ...
        dbPool.writeWithoutTransaction { db in
            ...
            XCTAssertEqual(db.description, "SQLEnclave.DatabasePool.writer")
            ...
            XCTAssertEqual(label, "SQLEnclave.DatabasePool.writer")
        }
        ...
        let block1 = { () in
            try! dbPool.read { db in
                XCTAssertEqual(db.configuration.label, nil)
                XCTAssertEqual(db.description, "SQLEnclave.DatabasePool.reader.1")
                
                // This test CAN break in future releases: the dispatch queue labels
                // are documented to be a debug-only tool.
                let label = String(utf8String: __dispatch_queue_get_label(nil))
                XCTAssertEqual(label, "SQLEnclave.DatabasePool.reader.1")
        ...
        let block2 = { () in
            ...
            try! dbPool.read { db in
                _ = s2.signal()
                XCTAssertEqual(db.configuration.label, nil)
                XCTAssertEqual(db.description, "SQLEnclave.DatabasePool.reader.2")
                
                // This test CAN break in future releases: the dispatch queue labels
                // are documented to be a debug-only tool.
                let label = String(utf8String: __dispatch_queue_get_label(nil))
                XCTAssertEqual(label, "SQLEnclave.DatabasePool.reader.2")
            }
        }
        let blocks = [block1, block2]
        DispatchQueue.concurrentPerform(iterations: blocks.count) { index in
