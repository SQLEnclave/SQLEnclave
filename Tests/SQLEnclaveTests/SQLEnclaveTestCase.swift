...
class SQLEnclaveTestCase: XCTestCase {
    ...
    override func setUp() {
        ...
        let dbPoolDirectoryName = "SQLEnclaveTestCase-\(ProcessInfo.processInfo.globallyUniqueString)"
        ...
        dbConfiguration.SQLiteConnectionWillClose = { sqliteConnection in
            ...
            //
            // SQLite would allow that. But not SQLEnclave, since all updates happen
    ...
    private func _assertEqualSQL<Request: FetchRequest>(_ db: Database, _ request: Request, _ sql: String, file: StaticString, line: UInt) throws {
        _ = try request.makeStatement(db).makeCursor().next()
