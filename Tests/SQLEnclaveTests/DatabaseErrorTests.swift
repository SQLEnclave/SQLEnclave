...
class DatabaseErrorTests: SQLEnclaveTestCase {
    ...
    func testNSErrorBridging() throws {
        ...
        try dbQueue.inDatabase { db in
            ...
            do {
                ...
                #if os(Linux)
                XCTSkip("failing on linux")
                #else
                XCTAssertEqual(DatabaseError.errorDomain, "SQLEnclave.DatabaseError")
                XCTAssertEqual((error as NSError).domain, DatabaseError.errorDomain)
                XCTAssertEqual((error as NSError).code, 787)
                #endif
            }
        }
    }
